...
class RideTest < ActiveSupport::TestCase
  ...
  test "should not save unless departlocation is filled in" do
	...
	assert ride.errors[:departlocation].include?("can't be blank")
  end
  
  test "should not save unless arrivelocation is filled in" do
    ride = Ride.new
	assert !ride.save # save should fails because there are errors.
	assert ride.errors[:arrivelocation].include?("can't be blank")
  end
  
  test "should not save unless departtime is filled in" do
    ride = Ride.new
	assert !ride.save # save should fails because there are errors.
	assert ride.errors[:departtime].include?("can't be blank")
  end
  
  test "should not save unless arrivetime is filled in" do
    ride = Ride.new
	assert !ride.save # save should fails because there are errors.
	assert ride.errors[:arrivetime].include?("can't be blank")
  end
  
  test "should not save unless seatsavailable is filled in" do
    ride = Ride.new
	assert !ride.save # save should fails because there are errors.
	assert ride.errors[:seatsavailable].include?("can't be blank")
  end
  
  test "should not save unless costperseat is filled in" do
    ride = Ride.new
	assert !ride.save # save should fails because there are errors.
	assert ride.errors[:costperseat].include?("can't be blank")
  end
